...
This crate does not impose a threading model of its own. The scheduler — the `Fetcher`
(`src/net/fetcher.rs`) — is an ordinary async component that the embedder drives on whatever Tokio
runtime it likes. A browser will typically run it on a dedicated runtime (its own thread) so that
network work never blocks the UI/main thread, but that wiring lives in the embedder, not in this
crate.

Using the fetcher comes down to three steps:

1. **Construct it** with `Fetcher::new(config, ctx)`. `config` is a `FetcherConfig` (connection
   limits, timeouts, user-agent, …) and `ctx` is an `Arc<dyn FetcherContext>` supplied by the
   embedder — see below.
2. **Run the loop.** Spawn `fetcher.run(shutdown)` as a long-lived task. This is the scheduler: it
   pulls requests from the priority queues, coalesces them, and spawns the actual fetch tasks. It
   parks on a `Notify` when idle and exits when the `shutdown` `CancellationToken` fires.
3. **Submit requests.** Call `fetcher.submit(req, handle, reply_tx)` to enqueue a `FetchRequest`,
   then await the matching `oneshot` receiver to get back a `FetchResult`.

The `FetcherContext` trait (`src/net/fetcher_context.rs`) is how the fetcher reaches back into its
environment without depending on any particular engine. It provides:

- `observer_for(...)` — hands back a `NetObserver` used to emit `NetEvent`s for a request.
- `on_ref_active` / `on_ref_done` — lifecycle callbacks fired when a unique fetch becomes active
  and when its last subscriber is done, so the embedder can track outstanding work.
- `is_url_allowed(url)` — a policy hook (SSRF guards, allow/block lists) consulted for the initial
  URL and every redirect target.

See `examples/fetcher.rs` for a minimal end-to-end setup.

Not every caller needs the scheduler. For standalone, one-shot requests there is a much smaller API
in `net::simple` (`src/net/simple.rs`): `simple_get` (async), plus `sync_get` and `sync_fetch`
(blocking helpers that each run on their own dedicated thread with its own Tokio runtime, so they
are safe to call even from inside another runtime). These skip coalescing, prioritisation and the
observer machinery entirely.
...
job — in either direction. The coalescing key (URL, method, headers, …) does not distinguish the two, so whether a 
request runs as a stream or as a buffer is decided by the *subscribers*, not by the key. The rule is simple: if **any** 
coalesced subscriber asked for streaming, the fetch is performed as a stream (`perform_streaming`), which produces a 
`SharedBody` — a fan-out stream that any number of consumers can read. Otherwise it is performed as a buffer 
(`perform_buffered`).
...
So, unlike a naive design, a buffered and a streaming request for the same resource never need two separate fetches.
